 ...
 * again. [server] and [address] are what a CalDAV client is given.
 */
...
class ConnectDeviceViewModel @Inject constructor(
    ...
    init {
        viewModelScope.launch {
            ...
            _uiState.value = _uiState.value.copy(server = server, address = "$server/calendars/caldav/")
        }
        load()
    }
    ...
    fun load() {
        viewModelScope.launch {
            ...
            try {
                // Device credentials only: an ICS link's token belongs to
                // one calendar and is replaced or revoked from its own menu.
