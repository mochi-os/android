...
/**
 * The shell's `menu/` permission endpoints, for the `permission_required`
 * answer to `-/calendars/subscribe`: the server asks the user to let the app
 * fetch from that host before it will subscribe. Authorised by the menu app's
 * own token, which the client mints at bootstrap.
 */
interface MenuApi {
