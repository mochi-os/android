...
private val FIELD_TYPE_KEYS =
    listOf("text", "number", "enumerated", "user", "date", "checkbox", "checklist")
...
fun CreateFieldScreen(
    ...
    CreateEntityScaffold(
        ...
        CreateEntityForm(padding) {
            ...
            Spacer(modifier = Modifier.height(8.dp))
            CheckboxRow(
                label = stringResource(R.string.crm_field_required),
                checked = isRequired,
                onCheckedChange = { checked -> isRequired = checked }
            )
            CheckboxRow(
                label = stringResource(R.string.crm_field_readonly),
                checked = isReadonly,
                onCheckedChange = { checked -> isReadonly = checked }
            )
            CheckboxRow(
                label = stringResource(R.string.crm_field_sortable),
                checked = isSortable,
                onCheckedChange = { checked -> isSortable = checked }
            )
            CheckboxRow(
                label = stringResource(R.string.crm_field_filterable),
                checked = isFilterable,
                onCheckedChange = { checked -> isFilterable = checked }
            )
            if (fieldtype == "enumerated") {
                CheckboxRow(
                    label = stringResource(R.string.crm_field_multi),
                    checked = isMulti,
