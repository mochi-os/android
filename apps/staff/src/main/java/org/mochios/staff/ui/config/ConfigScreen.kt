...
fun ConfigScreen(
    ...
    LaunchedEffect(Unit) {
        viewModel.events.collect { event ->
            when (event) {
                is ConfigEvent.Toast -> snackbarHostState.showSnackbar(resources.getString(event.messageRes))
                is ConfigEvent.Error -> {
                    val fallback = resources.getString(R.string.staff_config_toast_save_failed)
